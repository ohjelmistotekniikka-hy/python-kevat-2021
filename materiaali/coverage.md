...
[Invoke-ohjeessa](./invoke.md) ohjeistetaan, miten projektille voi määritellä uudelleenkäytettäviä tehtäviä ja miten niitä voi suorittaa komentoriviltä. Määritellään testikattavuuden keräämiselle ja raportin generoimiselle omat tehtävät, `coverage` ja `coverage-report`. Tämä onnistuu määritellemällä tehtävät projektin juurihakemiston _tasks.py_-tiedostossa:
